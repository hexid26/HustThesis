...
同时，使用低成本的 D2D 通信发送感知任务、收集感知数据，助于招募更多的参与者，还能减少主干网络的流量负载。
...
3）在边端融合场景下对移动群智感知的执行效率进行预测，并分析了不同边端资源调度对感知效率的影响。
...
图~\ref{Figure_MCS_Delay} 描述了边端融合场景下、，移动群智感知应用中各阶段的时间开销。
...
为了更直观比较不同边端资源对移动群智感知效率的影响，在此将 $N=5000$、$N_f=2$、$\lambda_n = 0.00003$、$\lambda_f=0.0004$ 做为图~\ref{Figure_SingleVal} 的基准测试场景（图中 base 实线）。
同样，感知应用的生命周期依然为100秒。
